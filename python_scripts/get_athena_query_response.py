...
def get_athena_query_response(
    sql_query, return_athena_types=False, timeout=None
):
    ...
    response = athena_client.start_query_execution(
        ...
        ResultConfiguration={"OutputLocation": out_path},
    )
    ...
    s3_path = athena_status["QueryExecution"]["ResultConfiguration"][
        "OutputLocation"
    ]
    if return_athena_types:
        meta = [
            {"name": c["Name"], "type": c["Type"]}
            for c in result_response["ResultSet"]["ResultSetMetadata"][
                "ColumnInfo"
            ]
        ]
    else:
        meta = [
            {"name": c["Name"], "type": type_dictionary[c["Type"]]}
            for c in result_response["ResultSet"]["ResultSetMetadata"][
                "ColumnInfo"
            ]
        ]
